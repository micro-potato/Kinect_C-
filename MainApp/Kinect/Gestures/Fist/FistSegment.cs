...
{
    public class FistSegment : IRelativeGestureSegment
    {
        ...
        public GesturePartResult CheckGesture(Body bodyData)
        {
            ...
            if(bodyData.HandRightState==HandState.Closed)
            {
                //Utils.LogHelper.GetInstance().ShowMsg("Distance from hand to shoulder:"+ (jointsData[JointType.ShoulderRight].Position.Z - jointsData[JointType.HandRight].Position.Z) * 1000);
                if ((jointsData[JointType.ShoulderRight].Position.Z - jointsData[JointType.HandRight].Position.Z)*1000>350)//Z方向伸出
                {
                    //Utils.LogHelper.GetInstance().ShowMsg("Distance from hand to shoulder:" + (Math.Abs(jointsData[JointType.HandRight].Position.X - jointsData[JointType.ShoulderRight].Position.X) * 1000));
